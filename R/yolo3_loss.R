transform_boxes_tf <- function(preds, anchors, n_class, net_h, net_w) {
  ...
  box_x = (k_sigmoid(box_x) + tf$cast(grid_col, tf$float32)) /  tf$cast(grid_w, tf$float32)
  box_y = (k_sigmoid(box_y) + tf$cast(grid_row, tf$float32)) /  tf$cast(grid_h, tf$float32)

  anchors_tf <- tf$Variable(anchors[[1]], tf$float32)
